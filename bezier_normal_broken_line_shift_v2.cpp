...
void bezierCurve(float x[], float y[])
{
	...
	//std::cout << "centerPoints: " << centerPoints.size() << std::endl;
	std::vector<std::pair<float, float>> shiftedPoints;

	float x1 = centerPoints[0].first;
	float y1 = centerPoints[0].second;
	float x2 = centerPoints[1].first;
	float y2 = centerPoints[1].second;
	...
	shiftedPoints.push_back(std::pair<float, float>(xa, ya));
	...
	//std::cout << "shiftedPoints: " << shiftedPoints.size() << std::endl;
					...
					if (event.type == SDL_MOUSEMOTION)
					{
						...
						change position of that control point to mouse new
								...
								if ((float)sqrt(abs(xnew - x[j]) * abs(xnew - x[j])
									+ abs(ynew - y[j]) * abs(ynew - y[j])) < 20.0)
								{
